...
const NavItems = ({ isShown, onClose }: Props) => {
  ...
  return (
    <div
      className={`${isShown ? "translate-x-0" : "translate-x-full"} fixed inset-0 z-40 transition-transform duration-300 lg:relative lg:block lg:translate-x-0`}
    >
      <div className="absolute bottom-0 left-0 top-0 z-40 flex w-[75%] flex-col rounded-r-[40px] bg-White px-6 pb-12 lg:relative lg:p-0">
        <div className="relative grid min-h-[103px] items-center py-8 lg:hidden">
          <Logo onClick={onClose} />
          <div className="absolute bottom-0 h-[2px] w-full rounded-sm bg-Gray" />
        </div>
        <div className="flex h-full flex-col gap-4 pt-8 lg:flex-row lg:gap-6 lg:p-0">
          {ROUTES.map((route) => (
            <NavLink key={route.id} data={route} onClick={onClose} />
          ))}
          {session && status === "authenticated" && (
            <div className="relative ml-6 hidden min-w-24 lg:block">
              ...
              >
                ...
                <button
                  ...
                  className="mt-1 w-full rounded-lg border border-red-400 px-3 py-2 text-sm text-red-500 hover:bg-red-50"
                >
                  Log out
                </button>
              </div>
          ...
          {status === "unauthenticated" && (
            <Button className="ml-6 hidden lg:block" href="/auth?mode=login" link>
              Log in
            </Button>
          )}
          {status === "loading" && <LoadingIndicator small className="ml-6" />}
        </div>
        <div className="relative flex flex-col gap-4 pt-6 lg:hidden">
          ...
          {session && status === "authenticated" && (
            <>
              <div className="">
                ...
                <button
                  onClick={logoutHandler}
                  className="flex w-full items-center gap-4 py-2 text-lg text-red-800 transition hover:text-red-500 lg:gap-1 lg:py-0"
                >
                  <BiLogOut className="text-2xl lg:hidden" />
      ...
      <div
        onClick={onClose}
        className="absolute bottom-0 right-0 top-0 z-30 w-[100%] bg-black/15 backdrop-blur-[2px] lg:hidden"
      />
    </div>
  );
};
